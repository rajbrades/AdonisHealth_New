/**
 * Lab PDF Extraction Test Script
 *
 * Tests the full pipeline:
 * 1. PDF text extraction using pdf-parse
 * 2. AI-powered biomarker extraction using Claude
 * 3. Matching against BiomarkerCatalog aliases
 *
 * Usage:
 *   npx ts-node scripts/test-lab-extraction.ts <path-to-pdf> [provider]
 *
 * Example:
 *   npx ts-node scripts/test-lab-extraction.ts ./test-labs/quest-sample.pdf QUEST
 */
...
interface MatchedBiomarker {
  ...
  matchedVia: string | null; // Which alias matched
...
async function matchBiomarkers(
  extracted: ExtractedBiomarker[],
  provider: string
): Promise<MatchedBiomarker[]> {
  console.log(`\n🔍 Matching biomarkers against catalog...`);

  // Get all aliases for this provider
  ...
  for (const ext of extracted) {
    ...
    if (exactAlias) {
      ...
      const fuzzyAlias = aliases.find(
        (a) =>
          a.aliasName.toLowerCase().includes(normalizedName) ||
      ...
      if (fuzzyAlias) {
        ...
        const directMatch = allBiomarkers.find(
          (b) =>
    ...
    // Calculate flag if we have a match and numeric value
    if (match.catalogMatch && match.numericValue !== null) {
      match.flag = calculateFlag(
        match.numericValue,
        match.catalogMatch.optimalRangeLow,
        match.catalogMatch.optimalRangeHigh
      );
    }
...
function parseNumericValue(value: string): number | null {
  ...
  // Remove < and > but keep the number
...
function calculateFlag(
  ...
  if (value < optimalLow * 0.8) return 'CRITICAL_LOW';
  if (value < optimalLow) return 'LOW';
  if (value > optimalHigh * 1.2) return 'CRITICAL_HIGH';
  if (value > optimalHigh) return 'HIGH';
  return 'OPTIMAL';
}

function printResults(matched: MatchedBiomarker[]): void {
  console.log(`\n${'='.repeat(100)}`);
  ...
  // Group by category
  const byCategory = new Map<string, MatchedBiomarker[]>();
  ...
  for (const [category, items] of byCategory) {
    ...
    for (const item of items) {
      ...
      console.log(
        `  ${flagEmoji} ${catalog.name.padEnd(35)} ${String(extracted.value).padStart(10)} ${(extracted.unit || catalog.defaultUnit).padEnd(12)} ` +
  ...
  if (unmatched.length > 0) {
    ...
    for (const item of unmatched) {
      console.log(
        `  • "${item.extracted.name}" = ${item.extracted.value} ${item.extracted.unit || ''}`
      );
    }
  }
  ...
  console.log(
    `  Match rate: ${((matched_count / matched.length) * 100).toFixed(1)}%`
  );
}
...
async function main() {
  ...
  // Check database connection and alias count
  ...
  try {
    ...
    // Step 2: Use AI to extract structured biomarker data
    const extracted = await extractBiomarkersWithAI(pdfText, provider);

    // Print extraction metadata
    if (extracted.collectionDate) console.log(`  Collection Date: ${extracted.collectionDate}`);
    if (extracted.patientName) console.log(`  Patient: ${extracted.patientName}`);

    // Step 3: Match against our biomarker catalog
