...
let BriefingService = BriefingService_1 = class BriefingService {
    prisma;
    anthropic = null;
    logger = new common_1.Logger(BriefingService_1.name);
    constructor(prisma) {
        this.prisma = prisma;
    ...
    async generateBriefingForAppointment(appointmentId) {
        if (!this.anthropic) {
        ...
        const labSummary = patient.labPanels.map(panel => {
            return `Date: ${panel.collectionDate.toISOString().split('T')[0]} - ${panel.panelName}\n` +
                panel.results.map(r => `- ${r.biomarker.name}: ${r.numericValue} ${r.normalizedUnit} (${r.flag || 'Normal'}) [Ref: ${r.refRangeLow}-${r.refRangeHigh}]`).join('\n');
        }).join('\n\n');
        const checkInSummary = patient.checkIns.map(c => `Date: ${c.date.toISOString().split('T')[0]} - Note: ${c.notes}\n` +
