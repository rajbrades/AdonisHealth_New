...
async function matchBiomarker(labName, provider) {
    ...
    const exactAlias = allAliases.find((a) => a.aliasName.toLowerCase().trim() === normalizedName);
    if (exactAlias) {
        return {
            labName,
...
async function analyzeLabResults() {
    ...
    console.log('\n📋 LABCORP - Ryan Brady');
    console.log('-'.repeat(80));
    const labcorpMatched = [];
    const labcorpUnmatched = [];
...
//# sourceMappingURL=analyze-sample-labs.js.map